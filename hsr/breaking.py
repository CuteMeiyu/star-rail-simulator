...
from game.flexflag import FlexFlag, MixFlag
from game.source import Source
from game.stats import *

from .buff import DebuffFlag
from .data import breaking as data
...
class BreakDamage(Damage):
    def __init__(self, source_unit: Unit, target_unit: Unit, element: ElementFlag, scale: float) -> None:
        super().__init__(source_unit, source_unit, target_unit, DamageFlag.breaking | element, BaseBreakDamageMultipier(scale), ToughnessMultipier(), BreakEffectMultipier())


def _deal_break_damage(source_unit: Unit, target_unit: Unit, element: ElementFlag, scale: float):
    BreakDamage(source_unit, target_unit, element, scale).deal()
...
def _on_weakness_break(event: EventWeaknessBreak):
    ...
    if ElementFlag.fire in element:
        ...
        _deal_break_damage(source_unit, target_unit, element, data.BreakScale.fire)
    elif ElementFlag.ice in element:
        Frozen(source_unit, target_unit).apply()
        _deal_break_damage(source_unit, target_unit, element, data.BreakScale.ice)
        ...
        _deal_break_damage(source_unit, target_unit, element, data.BreakScale.imaginary)
    elif ElementFlag.lightning in element:
        Shock(source_unit, target_unit).apply()
        _deal_break_damage(source_unit, target_unit, element, data.BreakScale.lightning)
    elif ElementFlag.physical in element:
        Bleed(source_unit, target_unit).apply()
        _deal_break_damage(source_unit, target_unit, element, data.BreakScale.physical)
        ...
        _deal_break_damage(source_unit, target_unit, element, data.BreakScale.quantum)
    elif ElementFlag.wind in element:
        WindShear(source_unit, target_unit).apply()
        _deal_break_damage(source_unit, target_unit, element, data.BreakScale.wind)


listen(EventWeaknessBreak, _on_weakness_break, Priority.Event.break_debuff)
